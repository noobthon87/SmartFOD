...
1. **New Scan** — enter job card number + task description, capture/upload a "before" photo and an "after" photo.
...
Mobile browsers block camera access (`st.camera_input`) on non-HTTPS origins other than `localhost`, so to use the camera on a phone you need an HTTPS deployment.
...
4. Deploy — you'll get a public `https://*.streamlit.app` URL that works with the camera on mobile.
